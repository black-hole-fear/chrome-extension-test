  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    function generateSave(url) { //creates the save button
      ...
      saveButton.onclick = async () => {
        await chrome.downloads.download({url: url, filename: `${currentDate}.${format}`, saveAs: true});
        chrome.runtime.sendMessage({ type: "RECORD_STOPPED" });
      };
      saveButton.style.display = "inline-block";
    }
  });
  review.onclick = () => {
    chrome.tabs.create({url: "https://chrome.google.com/webstore/detail/chrome-capture-extension/kfokdmfpdnokpmpbjhjbcabgligoelgp/reviews"});
  }
